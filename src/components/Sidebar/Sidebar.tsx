...
const Sidebar: React.FC = () => {
  ...
  const navItems: NavItem[] = [
    ...
    { to: '/sales', icon: <ShoppingCart className="h-5 w-5" />, label: 'Sales', badge: 5 },
    { to: '/inventory', icon: <Package className="h-5 w-5" />, label: 'Inventory' },
    { to: '/finance', icon: <DollarSign className="h-5 w-5" />, label: 'Finance' },
    { to: '/customers', icon: <Users className="h-5 w-5" />, label: 'Customers' },
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 p-2 bg-gray-200 dark:bg-gray-700 rounded-md md:hidden"
      >
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-16 h-[calc(100vh-4rem)] w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 overflow-y-auto transition-transform duration-300 z-40 
